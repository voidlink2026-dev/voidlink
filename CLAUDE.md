...
- `PRODUCTION.md` — handbook for production-side work that ships outside the binary: music self-production (FL Studio, instruments, mastering), voice acting (Voquent, casting briefs), trailer workflow (DaVinci Resolve, Derek Lieu formula, structure). UK-focused resource directory. Update when a supplier is contacted, hired, or rejected. Maps 1:1 against L1 (audio) and L7 (trailer + press kit) sprints in `Next_Stage.md`.
- `casting/` — operational folder for the voice-acting workstream. Contains `NDA.md` (mutual NDA template for talent shortlists), `README.md` (workflow index), and `scripts/01_cipher.md` through `scripts/07_voidlink_dispatch.md` (per-character recording scripts with auditions, cue numbers, voice direction, pickup terms). Send NDA before sharing scripts. Scripts pull canonical letter content from the codebase where it exists and write fresh dialogue for ending-only characters (Kovac, Mei Lin). Voidlink Dispatch is optional and only shipped if Tier C voicing or above per `PRODUCTION.md`.
