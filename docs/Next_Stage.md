...
**Effort:** ~3 hours. **Window:** any pre-EA window.

A one-line dynamic title in the Profile window that updates as the player accumulates choices. Reads from `getDecisionPattern()` + dominant traits. Visible *only* in Profile (never on a leaderboard, never on a Steam achievement).

**Examples generated by pattern bucket:**
- strong_principled + bond_clean dominant: *"You are: the kind of operative Cipher writes to."*
- weak_mercenary + bond_collaborator: *"You are: a quiet professional with three uncomfortable secrets."*
- strong_mercenary + magnus_collaborator: *"You are: the one who answered when something older asked."*
- bond_resistor + balance_keeper: *"You are: the one who chose to be complicit in a managed war."*
- isReformer = true: *"You are: someone who turned away."*

**Tech.**
- `libs/core/src/data/operativeSignature.ts` — a table of (predicate → signature line) entries, evaluated top-to-bottom (specific traits first, then fallback).
- `getOperativeSignature(player)` returns the matched string.
- `ProfileWindow` renders it under the operative handle.

**Acceptance.** ~30 signature variants. Each is testable by setting flags via devtools. Default for a fresh player: *"You are: an operative."*
