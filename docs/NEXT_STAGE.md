...
| **M14h.3** ✅ | UX polish batch: modern Data Globe background (Fibonacci dot-cluster + cyan palette + hub pulses + arc connections, replaces wireframe), tutorial auto-focuses spotlit windows, bounce→RELAY rename (user-visible labels), node colour state shows YELLOW when scanned + GREEN when breached, 3-pulse intruder beep when rival hacker spawns | Tier 1 | SHIPPED 2199-01-01 |
